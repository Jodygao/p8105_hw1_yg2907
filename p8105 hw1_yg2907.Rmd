...
scatterplot <- ggplot(early_january_weather, aes(x = time_hour, y = temp, color = humid)) +
  ...
  labs(
    ...
    y = "Temperature",
    title = "Scatterplot of Temperature vs. Time with Humidity Color",
    subtitle = "Early January Weather Data"
  )
...
# Export Graph
ggsave("temperature_vs_time_scatterplot.png", plot = scatterplot, width = 8, height = 6)
...
# Mean
mean_normal <- mean(pull(my_data, normal_sample))
mean_logical <- mean(pull(my_data, logical_vector))
mean_character <- mean(pull(my_data, character_vector))
mean_factor <- mean(pull(my_data, factor_vector))

# To numeric
numeric_logical <- as.numeric(my_data$logical_vector)
numeric_character <- as.numeric(my_data$character_vector)
